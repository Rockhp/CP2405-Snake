...
<html>
  <head>
    ...
    <title>A Newbie Guide to pygame &#8212; pygame v2.1.2 documentation</title>
...
platforms (Windows, Unix, Mac, BeOS and others).</p>
...
interpreter and play with the problem for a few hours.  Print out the <a class="reference external" href="http://www.brunningonline.net/simon/python/quick-ref2_0.html">Python
2.0 Quick Reference</a> and keep it by your computer.</p>
...
<code class="docutils literal notranslate"><span class="pre">Userrect</span></code>, <code class="docutils literal notranslate"><span class="pre">surfarray</span></code> or <code class="docutils literal notranslate"><span class="pre">version</span></code> functions.</p>
</section>
...
<code class="docutils literal notranslate"><span class="pre">pygame.display.set_mode()</span></code>.  This 'display surface' represents the screen;
whatever you do to it will appear on the user's screen.  You can only have one
of these -- that's an SDL limitation, not a pygame one.</p>
...
that contains an image by using <code class="docutils literal notranslate"><span class="pre">image.load()</span></code>, or you can make a surface
that contains text with <code class="docutils literal notranslate"><span class="pre">font.render()</span></code>.  You can even create a surface that
contains nothing at all with <code class="docutils literal notranslate"><span class="pre">Surface()</span></code>.</p>
<p>Most of the surface functions are not critical. Just learn <code class="docutils literal notranslate"><span class="pre">blit()</span></code>,
<code class="docutils literal notranslate"><span class="pre">fill()</span></code>, <code class="docutils literal notranslate"><span class="pre">set_at()</span></code> and <code class="docutils literal notranslate"><span class="pre">get_at()</span></code>, and you'll be fine.</p>
</section>
...
<p>When I first read the documentation for <code class="docutils literal notranslate"><span class="pre">surface.convert()</span></code>, I didn't think
...
<p>The 'format' that <code class="docutils literal notranslate"><span class="pre">convert()</span></code> refers to isn't the <em>file</em> format (ie PNG,
...
<code class="docutils literal notranslate"><span class="pre">image.load()</span></code> function. Instead of just doing:</p>
...
<section id="dirty-rect-animation">
<h3>Dirty rect animation.<a class="headerlink" href="#dirty-rect-animation" title="Permalink to this headline">¶</a></h3>
<p>The most common cause of inadequate frame rates in pygame programs results from
misunderstanding the <code class="docutils literal notranslate"><span class="pre">pygame.display.update()</span></code> function.  With pygame, merely
drawing something to the display surface doesn't cause it to appear on the
screen -- you need to call <code class="docutils literal notranslate"><span class="pre">pygame.display.update()</span></code>.  There are three ways
of calling this function:</p>
<blockquote>
<div><ul class="simple">
<li><p><code class="docutils literal notranslate"><span class="pre">pygame.display.update()</span></code> -- This updates the whole window (or the whole screen for fullscreen displays).</p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">pygame.display.flip()</span></code> -- This does the same thing, and will also do the right thing if you're using <code class="docutils literal notranslate"><span class="pre">double-buffered</span></code> hardware acceleration, which you're not, so on to...</p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">pygame.display.update(a</span> <span class="pre">rectangle</span> <span class="pre">or</span> <span class="pre">some</span> <span class="pre">list</span> <span class="pre">of</span> <span class="pre">rectangles)</span></code> -- This updates just the rectangular areas of the screen you specify.</p></li>
</ul>
</div></blockquote>
<p>Most people new to graphics programming use the first option -- they update the
whole screen every frame.  The problem is that this is unacceptably slow for
most people.  Calling <code class="docutils literal notranslate"><span class="pre">update()</span></code> takes 35 milliseconds on my machine, which
doesn't sound like much, until you realize that 1000 / 35 = 28 frames per
second <em>maximum</em>. And that's with no game logic, no blits, no input, no AI,
nothing.  I'm just sitting there updating the screen, and 28 fps is my maximum
framerate. Ugh.</p>
<p>The solution is called 'dirty rect animation'.  Instead of updating the whole
...
calling <code class="docutils literal notranslate"><span class="pre">update(the_dirty_rectangles)</span></code> at the end of the frame.  In detail
for a moving sprite, I:</p>
<blockquote>
<div><ul class="simple">
...
<p>The difference in speed is astonishing. Consider that <a class="reference external" href="https://www.pygame.org/shredwheat/solarwolf/index.shtml">SolarWolf</a> has dozens of
constantly moving sprites updating smoothly, and still has enough time left
over to display a parallax starfield in the background, and update that too.</p>
...
background jumping along too much.</p>
<p>A final note -- not every game requires high framerates. A strategic wargame
could easily get by on just a few updates per second -- in this case, the added
complexity of dirty rect animation may not be necessary.</p>
</section>
...
<section id="hardware-surfaces-are-more-trouble-than-they-re-worth">
<h3>Hardware surfaces are more trouble than they're worth.<a class="headerlink" href="#hardware-surfaces-are-more-trouble-than-they-re-worth" title="Permalink to this headline">¶</a></h3>
<p><strong>Especially in pygame 2, because HWSURFACE now does nothing</strong></p>
<p>If you've been looking at the various flags you can use with
<code class="docutils literal notranslate"><span class="pre">pygame.display.set_mode()</span></code>, you may have thought like this: <cite>Hey,
HWSURFACE! Well, I want that -- who doesn't like hardware acceleration. Ooo...
DOUBLEBUF; well, that sounds fast, I guess I want that too!</cite>.  It's not
your fault; we've been trained by years of 3-d gaming to believe that hardware
acceleration is good, and software rendering is slow.</p>
<p>Unfortunately, hardware rendering comes with a long list of drawbacks:</p>
<blockquote>
<div><ul class="simple">
<li><p>It only works on some platforms. Windows machines can usually get hardware surfaces if you ask for them. Most other platforms can't. Linux, for example, may be able to provide a hardware surface if X4 is installed, if DGA2 is working properly, and if the moons are aligned correctly. If a hardware surface is unavailable, SDL will silently give you a software surface instead.</p></li>
<li><p>It only works fullscreen.</p></li>
<li><p>It complicates per-pixel access.  If you have a hardware surface, you need to Lock the surface before writing or reading individual pixel values on it.  If you don't, Bad Things Happen. Then you need to quickly Unlock the surface again, before the OS gets all confused and starts to panic.  Most of this process is automated for you in pygame, but it's something else to take into account.</p></li>
<li><p>You lose the mouse pointer. If you specify <code class="docutils literal notranslate"><span class="pre">HWSURFACE</span></code> (and actually get it), your pointer will usually just vanish (or worse, hang around in a half-there, half-not flickery state).  You'll need to create a sprite to act as a manual mouse pointer, and you'll need to worry about pointer acceleration and sensitivity. What a pain.</p></li>
<li><p>It might be slower anyway. Many drivers are not accelerated for the types of drawing that we do, and since everything has to be blitted across the video bus (unless you can cram your source surface into video memory as well), it might end up being slower than software access anyway.</p></li>
</ul>
</div></blockquote>
<p>Hardware rendering has its place. It works pretty reliably under Windows, so
...
<code class="docutils literal notranslate"><span class="pre">SWSURFACE</span></code> until you're sure you know what you're doing.</p>
</section>
...
but I have to admit my favorite part of pygame is the lowly <code class="docutils literal notranslate"><span class="pre">Rect</span></code> class.  A
...
<p>So you've got your sprites moving around, and you need to know whether or not they're bumping into one another. It's tempting to write something like the following:</p>
<blockquote>
<div><ul class="simple">
...
calling, say, <code class="docutils literal notranslate"><span class="pre">pygame.mouse.get_pos()</span></code> or <code class="docutils literal notranslate"><span class="pre">pygame.key.get_pressed()</span></code>.
...
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="k">if</span> <span class="p">(</span><span class="n">key</span><span class="o">.</span><span class="n">get_pressed</span><span class="p">[</span><span class="n">K_t</span><span class="p">]</span> <span class="ow">and</span> <span class="n">key</span><span class="o">.</span><span class="n">get_pressed</span><span class="p">[</span><span class="n">K_f</span><span class="p">]):</span>
    <span class="nb">print</span> <span class="s2">&quot;Yup!&quot;</span>
</pre></div>
</div>
...
have much going on in your loop -- say you're just sitting in a <code class="docutils literal notranslate"><span class="pre">while</span> <span class="pre">1</span></code>
...
rectangular in shape.  Simply call <code class="docutils literal notranslate"><span class="pre">surface.set_colorkey(color)</span></code>, where
color is an RGB tuple -- say (0,0,0). This would make every pixel in the source
image transparent instead of black.</p>
...
unwieldy, chances are its speed can be improved, too.  Read over <a class="reference external" href="http://www-rohan.sdsu.edu/~gawron/compling/course_core/python_intro/intro_lecture_files/fastpython.html">Python
Performance Tips</a> for some great advice on how you can improve the speed of
...
things are just not meant to be :)</p>
